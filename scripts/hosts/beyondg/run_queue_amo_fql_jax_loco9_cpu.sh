#!/usr/bin/env bash
# CPU failover for the live AMO-fql JAX loco9 T-init-5 queues.
...
if pgrep -af "launch_fql_amo_jax_loco9_tinit5_alrgrid.py" | grep -v -- '--device=cpu' | grep -v grep >/dev/null 2>&1; then
...
raise SystemExit(0 if b"launch_fql_amo_jax_loco9" in cmd else 1)' "$out/launcher_process.json" 2>/dev/null; then
