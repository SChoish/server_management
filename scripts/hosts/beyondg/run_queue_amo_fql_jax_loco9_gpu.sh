...
# AntMaze priority: refuse to reclaim GPUs while this marker exists.
if [[ -f "$STORE/PAUSE_FQL_LOCO9" ]]; then
  echo "PAUSE_FQL_LOCO9 present; loco9 GPU queue suppressed (antmaze priority)"
  exit 0
fi
